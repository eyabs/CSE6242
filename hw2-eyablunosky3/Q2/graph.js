...
var path = svg.append("svg:g").selectAll("path")
  ...
  .enter().append("svg:path")
    ...
    .classed("red", function(d) {return (d.value=="1");})
    .classed("blue", function(d) {return (d.value=="0");});
...
var radiusScale = d3.scale.linear()
  .range([5, 15])
  .domain([0, d3.max(d3.values(nodes), function(d){return d.weight;})]);

// add the nodes
node.append("circle")
    .attr("r", function(d) {return radiusScale(d.weight) || "5";});
node.append("text")
    .text(function(d){return d.name})

function pinNode(d) {
      // d3.select(this).select("circle").classed("fixed", 
      //   !d3.select(this).select("circle").classed("fixed"));
